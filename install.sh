...
echo ""
last_version=$(curl -Ls "https://www.ghproxy.cn/https://api.github.com/repos/gm-cx/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
# 获取 x-ui 版本
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://www.ghproxy.cn/https://api.github.com/repos/xeefei/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        echo ""
        sleep 2
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://www.ghproxy.cn/https://github.com/xeefei/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://www.ghproxy.cn/https://github.com/xeefei/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz"
        echo ""
        echo -e "--------------------------------------------"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://www.ghproxy.cn/https://raw.githubusercontent.com/xeefei/3x-ui/main/x-ui.sh
